...
# Global Config
app = FastAPI(
    title=fahem_config.site_name,
    description=fahem_config.site_description,
    version="0.1.0",
)

app.add_middleware(
    CORSMiddleware,
    allow_origin_regex=fahem_config.hosting_config.allowed_regexp,
    allow_methods=["*"],
...
if __name__ == "__main__":
    uvicorn.run(
        "app:app",
        host="0.0.0.0",
        port=fahem_config.hosting_config.port,
        reload=fahem_config.general_config.development_mode,
    )
...
@app.get("/")
async def root():
    return {"Message": "Welcome to Fahem ✨"}
