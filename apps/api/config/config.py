...
class FahemLmsConfig(BaseModel):
    site_name: str
...
def get_fahem_config() -> FahemLmsConfig:
    ...
    env_development_mode = eval(os.environ.get("FAHEM_DEVELOPMENT_MODE", "None"))
    development_mode = (
    ...
    env_install_mode = os.environ.get("FAHEM_INSTALL_MODE", "None")
    install_mode = (
        env_install_mode
    ...
    # Security Config
    env_auth_jwt_secret_key = os.environ.get("FAHEM_AUTH_JWT_SECRET_KEY")
    auth_jwt_secret_key = env_auth_jwt_secret_key or yaml_config.get(
        "security", {}
    ).get("auth_jwt_secret_key")

    # Check if environment variables are defined
    env_site_name = os.environ.get("FAHEM_SITE_NAME")
    env_site_description = os.environ.get("FAHEM_SITE_DESCRIPTION")
    env_contact_email = os.environ.get("FAHEM_CONTACT_EMAIL")
    env_domain = os.environ.get("FAHEM_DOMAIN")
    os.environ.get("FAHEM_PORT")
    env_ssl = os.environ.get("FAHEM_SSL")
    env_port = os.environ.get("FAHEM_PORT")
    env_use_default_org = os.environ.get("FAHEM_USE_DEFAULT_ORG")
    env_allowed_origins = os.environ.get("FAHEM_ALLOWED_ORIGINS")
    env_cookie_domain = os.environ.get("FAHEM_COOKIE_DOMAIN")

    # Allowed origins should be a comma separated string
    if env_allowed_origins:
        env_allowed_origins = env_allowed_origins.split(",")
    env_allowed_regexp = os.environ.get("FAHEM_ALLOWED_REGEXP")
    env_self_hosted = os.environ.get("FAHEM_SELF_HOSTED")
    env_sql_connection_string = os.environ.get("FAHEM_SQL_CONNECTION_STRING")

    # Sentry Config
    env_sentry_dsn = os.environ.get("FAHEM_SENTRY_DSN")
    env_sentry_environment = os.environ.get("FAHEM_SENTRY_ENVIRONMENT")
    env_sentry_release = os.environ.get("FAHEM_SENTRY_RELEASE")
    ...
    env_content_delivery_type = os.environ.get("FAHEM_CONTENT_DELIVERY_TYPE")
    content_delivery_type: str = env_content_delivery_type or (
        (yaml_config.get("hosting_config", {}).get("content_delivery", {}).get("type"))
        or "filesystem"
    )  # default to filesystem

    env_bucket_name = os.environ.get("FAHEM_S3_API_BUCKET_NAME")
    env_endpoint_url = os.environ.get("FAHEM_S3_API_ENDPOINT_URL")
    bucket_name = (
    ...
    # Redis config
    env_redis_connection_string = os.environ.get("FAHEM_REDIS_CONNECTION_STRING")
    redis_connection_string = env_redis_connection_string or yaml_config.get(
        "redis_config", {}
    ).get("redis_connection_string")

    # Mailing config
    env_resend_api_key = os.environ.get("FAHEM_RESEND_API_KEY")
    env_system_email_address = os.environ.get("FAHEM_SYSTEM_EMAIL_ADDRESS")
    ...
    # Create FahemLmsConfig object
    config = FahemLmsConfig(
        site_name=site_name,
